...
const migrationUrl = new URL("../../database/modules/pos/migrations/POS-0008-receipt-delivery.sql", import.meta.url);
...
test("POS-0008 receipt delivery is immutable, tenant-isolated and permission-scoped", async () => {
  ...
  assert.match(migration, /CREATE TABLE pos\.receipt_delivery_requests/u);
  assert.match(migration, /receipt_delivery_requests_append_only/u);
  assert.match(migration, /ENABLE ROW LEVEL SECURITY/u);
  assert.match(migration, /FORCE ROW LEVEL SECURITY/u);
  assert.match(migration, /CREATE POLICY tenant_isolation/u);
  assert.match(migration, /UNIQUE \(tenant_id, receipt_snapshot_id, request_id\)/u);
  assert.match(migration, /SECURITY DEFINER/u);
  assert.match(migration, /REVOKE ALL ON FUNCTION pos\.request_receipt_delivery_v1/u);
  assert.match(migration, /GRANT EXECUTE ON FUNCTION pos\.request_receipt_delivery_v1/u);
  assert.match(migration, /pos\.receipt\.delivery\.requested\.v1/u);
  assert.match(migration, /platform\.audit_events/u);
  assert.match(migration, /platform\.outbox_events/u);
});
