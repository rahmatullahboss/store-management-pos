...
test("POS migration preserves checkout, receipt, sync and device evidence", async () => {
  ...
  assert.match(sql, /CREATE TABLE pos\.carts/u);
  assert.match(sql, /CREATE TABLE pos\.cart_lines/u);
  assert.match(sql, /FOREIGN KEY \(tenant_id, cart_id\) REFERENCES pos\.carts/u);
  assert.match(sql, /offline_authorizations_signature_present/u);
  assert.match(sql, /permission_snapshot_version/u);
  assert.match(sql, /country_capability_version/u);
  assert.match(sql, /CREATE TABLE pos\.offline_operations/u);
  assert.match(sql, /offline_operations_append_only/u);
  assert.match(sql, /CREATE TABLE pos\.offline_operation_outcomes/u);
