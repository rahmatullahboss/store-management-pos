...
function releaseNotice(data: StagingOperationalData, version: string): string {
  return `<section data-staging-notice role="status" style="margin:0 0 1rem;padding:.85rem 1rem;background:#fff0c7;color:#4c3100;border-radius:14px"><strong>Usable release candidate · synthetic data</strong><span style="display:block;margin-top:.25rem">Signed in as <strong>${escapeHtml(data.context.user.name)}</strong> · ${escapeHtml(data.context.role)} · ${data.context.permissions.length} database-resolved read permissions</span><span style="display:block;margin-top:.2rem">Operational reads are live. Checkout, payment, stock, order, accounting and banking commands remain disabled.</span><form action="/auth/sign-out" method="post" style="margin-top:.6rem"><button type="submit" style="min-height:40px;border:1px solid currentColor;background:transparent;color:inherit;padding:.4rem .75rem;font:800 .86rem system-ui;cursor:pointer">Sign out</button></form><small style="display:block;margin-top:.4rem">Build ${escapeHtml(version)}</small></section>`;
}
...
export async function handleOperationalStagingRequest(
  ...
  const data = await loadStagingOperationalData(env.DATABASE_URL, context);
