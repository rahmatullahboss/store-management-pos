...
export default {
  async fetch(request: Request, env: ApiEnvironment): Promise<Response> {
    ...
    try {
      ...
      const observeFinance = async (
        module: "payment" | "accounting" | "banking" | "finance",
        operation: string,
        work: () => Promise<Response>,
      ): Promise<Response> => await observeFinanceOperation(context, financeObserver, module, operation, work);
      ...
      if (request.method === "POST" && url.pathname === "/v1/payments/intents") {
        return await observeFinance("payment", "intent.create", async () => await handleCreatePaymentIntent(request, context, database, env));
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/refunds") {
        return await observeFinance("payment", "refund.create", async () => await handleCreateRefund(request, context, database, env));
      }
      if (request.method === "POST" && url.pathname === "/v1/settlements/import") {
        return await observeFinance("payment", "settlement.import", async () => await handleImportSettlement(request, context, database, env));
      }
      if (request.method === "POST" && url.pathname === "/v1/accounting/journals") {
        return await observeFinance("accounting", "journal.post", async () => await handlePostJournal(request, context, database));
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/accounting/open-items") {
        return await observeFinance("accounting", "open_item.create", async () => await handleCreateOpenItem(request, context, database));
      }
      ...
      if (request.method === "GET" && url.pathname === "/v1/accounting/reports/trial-balance") {
        return await observeFinance("accounting", "report.trial_balance", async () => await handleTrialBalance(url, context, database));
      }
      if (request.method === "GET" && url.pathname === "/v1/accounting/reports/general-ledger") {
        return await observeFinance("accounting", "report.general_ledger", async () => await handleGeneralLedger(url, context, database));
      }
      if (request.method === "GET" && url.pathname === "/v1/accounting/reports/open-item-aging") {
        return await observeFinance("accounting", "report.open_item_aging", async () => await handleOpenItemAging(url, context, database));
      }
      if (request.method === "POST" && url.pathname === "/v1/banking/statements/import") {
        return await observeFinance("banking", "statement.import", async () => await handleImportBankStatement(request, context, database));
      }
      if (request.method === "POST" && url.pathname === "/v1/banking/reconciliations") {
        return await observeFinance("banking", "reconciliation.match", async () => await handleReconcileStatementLine(request, context, database));
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/banking/reconciliation-runs") {
        return await observeFinance("banking", "reconciliation.run", async () => await handleRecordReconciliationRun(request, context, database));
      }
      if (request.method === "GET" && url.pathname === "/v1/banking/unreconciled") {
        return await observeFinance("banking", "reconciliation.unreconciled", async () => await handleListUnreconciled(url, context, database));
      }
      if (request.method === "GET" && url.pathname === "/v1/finance/readiness") {
        return await observeFinance("finance", "readiness.read", async () => await handleFinanceReadiness(context, database));
      }
