...
function quantity(raw: string, scale: number): string {
...
async function workspaceData(
  ...
  if (!warehouseId) throw new PlatformError("PERMISSION_DENIED", "Warehouse scope is required", 403);
  ...
  const [variants, reservations, factors] = await Promise.all([
    database.httpQuery<VariantRow>(
      `SELECT variant.id::text AS variant_id,
    ...
    database.httpQuery(
      `SELECT id FROM platform.auth_mfa_factors
       WHERE user_id = $1::uuid AND status = 'active' LIMIT 1`,
      [context.user.id],
    ),
  ]);
...
function feedback(url: URL): string {
  ...
  if (error) return `<p class="error" role="alert">${escapeHtml(error.slice(0, 180))}</p>`;
  for (const key of ["created", "released"] as const) {
    const id = url.searchParams.get(key);
    if (id && UUID.test(id)) {
      return `<p class="success" role="status">Reservation ${escapeHtml(id)} was ${key === "created" ? "created" : "released"}. The step-up grant was consumed.</p>`;
    }
  }
...
function render(
  url: URL,
  context: StagingReadContext,
  data: Awaited<ReturnType<typeof workspaceData>>,
): string {
  const available = data.variants.filter((row) => BigInt(row.available || "0") > 0n);
  const options = available.length === 0
    ? '<option value="">No available EACH variants</option>'
    : available.map((row) =>
      `<option value="${escapeHtml(row.variant_id)}">${escapeHtml(row.product_name)} · ${escapeHtml(row.variant_title)} · ${escapeHtml(row.sku)} · ${escapeHtml(row.available)} available</option>`,
    ).join("");
  const rows = data.reservations.length === 0
    ...
    : data.reservations.map((row) => {
      const releasable = ["fully_reserved", "partially_reserved", "partially_consumed"].includes(row.state);
      return `<tr><td><code>${escapeHtml(row.source_id)}</code></td><td>${escapeHtml(row.sku)}</td><td>${escapeHtml(quantity(row.requested_quantity, row.quantity_scale))} ${escapeHtml(row.unit_code)}</td><td>${escapeHtml(quantity(row.reserved_quantity, row.quantity_scale))}</td><td>${escapeHtml(row.state.replaceAll("_", " "))}</td><td>${escapeHtml(row.version)}</td><td>${releasable ? `<form action="${WORKSPACE}/${escapeHtml(row.reservation_id)}/release" method="post"><input type="hidden" name="expectedVersion" value="${escapeHtml(row.version)}"><button class="danger" type="submit">Release with step-up</button></form>` : "Completed"}</td></tr>`;
    }).join("");
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>Controlled reservations</title><style>:root{font-family:ui-sans-serif,system-ui;color:#17231e;background:#f5f3ec}*{box-sizing:border-box}body{margin:0}.top{padding:1rem;background:#14251e;color:#fff}.top a{color:#f0d36d}.shell{width:min(100%,76rem);margin:0 auto;padding:clamp(1rem,3vw,2rem)}h1{font-size:clamp(2rem,6vw,4rem);line-height:.95;letter-spacing:-.04em}.grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(18rem,.7fr);gap:1rem}.card{background:#fffefa;border:1px solid #d7ddd8;border-radius:1rem;padding:1.25rem}.notice,.success,.error{padding:.8rem 1rem;border-radius:.65rem}.notice,.success{border:2px solid #1f6a51;background:#edf8f3}.error{border:2px solid #9b2c2c;background:#fff2f0;color:#762020}.badge{display:inline-block;padding:.3rem .55rem;border-radius:999px;font-weight:800;background:#fff0c7}.badge.good{background:#d9f2e7}form{display:grid;gap:.75rem}label{font-weight:800}input,select{width:100%;min-height:48px;padding:.7rem;border:1px solid #87928b;border-radius:.45rem;font:inherit}button,.button{display:inline-flex;align-items:center;justify-content:center;min-height:44px;border:0;border-radius:.45rem;background:#14251e;color:#fff;padding:.6rem .9rem;font-weight:800;text-decoration:none;cursor:pointer}.secondary{background:#fff;color:#14251e;border:1px solid #14251e}.danger{background:#7b2d26}.actions{display:flex;gap:.65rem;flex-wrap:wrap;margin:1rem 0}.table-wrap{overflow-x:auto;background:#fffefa;border:1px solid #d7ddd8;border-radius:1rem}table{width:100%;border-collapse:collapse;min-width:54rem}th,td{text-align:left;padding:.75rem;border-bottom:1px solid #e2e6e3;vertical-align:top}th{background:#eef1ee}td form{display:block}@media(max-width:760px){.grid{grid-template-columns:1fr}}</style></head><body><header class="top"><div class="shell"><strong>Persistent staging · controlled reservation checkpoint</strong><nav><a href="/admin">Dashboard</a> · <a href="/admin/inventory">Inventory</a> · <a href="/auth/mfa">MFA settings</a></nav></div></header><main class="shell"><p>${escapeHtml(context.tenant.name)} · ${escapeHtml(context.user.name)} · warehouse-scoped</p><h1>Controlled reservations</h1><p class="notice">This is the only enabled sensitive command. Every create or release requires current password + TOTP and a single-use grant. Payments, stock postings, transfers, accounting and banking remain disabled.</p>${feedback(url)}<div class="actions"><span class="badge ${data.mfaEnrolled ? "good" : ""}">${data.mfaEnrolled ? "TOTP MFA active" : "MFA setup required"}</span><a class="button secondary" href="/auth/mfa">${data.mfaEnrolled ? "Verify MFA step-up" : "Set up MFA"}</a></div><section class="grid"><div class="card"><h2>Create one reservation</h2><form action="${CREATE}" method="post"><div><label for="variant">Variant</label><select id="variant" name="variantId" required>${options}</select></div><div><label for="source">Reference</label><input id="source" name="sourceId" value="manual-${Date.now()}" maxlength="100" required></div><div><label for="qty">Quantity</label><input id="qty" name="quantity" type="number" min="1" max="5" step="1" value="1" required></div><button type="submit" ${available.length === 0 ? "disabled" : ""}>Create with step-up</button></form></div><div class="card"><h2>Security state</h2><dl><dt>Session role</dt><dd>${escapeHtml(context.role)}</dd><dt>Read permissions</dt><dd>${context.permissions.length}</dd><dt>MFA</dt><dd>${data.mfaEnrolled ? "Active" : "Not enrolled"}</dd><dt>Command</dt><dd><code>inventory.reservation.manage</code></dd><dt>Grant</dt><dd>Single-use, max 5 minutes</dd><dt>Command token</dt><dd>Internal only, max 60 seconds</dd></dl></div></section><h2>Recent reservations</h2><div class="table-wrap"><table><thead><tr><th>Reference</th><th>SKU</th><th>Requested</th><th>Reserved</th><th>State</th><th>Version</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div></main></body></html>`;
}
...
async function commandRedirect(
  ...
  if (response.ok) {
    try {
      ...
      if (typeof result.id !== "string" || !UUID.test(result.id)) throw new Error("invalid response");
      target.searchParams.set(successKey, result.id);
    } catch {
  ...
  return redirect(request, `${target.pathname}${target.search}`, response.headers.get("set-cookie"));
}
...
async function createReservation(
  ...
  if (!context.scope.warehouseId) throw new PlatformError("PERMISSION_DENIED", "Warehouse scope is required", 403);
  ...
  if (!UUID.test(variantId) || sourceId.length < 1 || sourceId.length > 100 || !Number.isInteger(requested) || requested < 1 || requested > 5) {
    throw new PlatformError("VALIDATION_FAILED", "Reservation form input is invalid", 400);
  }
  const response = await protectedRequest(request, env, "/api/v1/inventory/reservations", {
    ...
    lines: [{
      ...
      quantity: { amount: String(requested), unit: "EACH", scale: 0 },
    }],
  });
  return await commandRedirect(request, response, "created");
}
...
async function releaseReservation(
  ...
  return await commandRedirect(request, response, "released");
}
...
export async function handleStagingReservationUi(
  ...
  const isWorkspace = url.pathname === WORKSPACE || url.pathname === `${WORKSPACE}/`;
  const isCreate = url.pathname === CREATE;
  const releaseMatch = RELEASE.exec(url.pathname);
  if (!isWorkspace && !isCreate && !releaseMatch) return null;
  try {
    const context = await resolveStagingReadContext(request, env);
    if (!context) return redirect(request, `/login?returnTo=${encodeURIComponent(WORKSPACE)}`);
    if (isWorkspace && (request.method === "GET" || request.method === "HEAD")) {
      return htmlResponse(request, render(url, context, await workspaceData(env, context)));
    }
    if (isCreate && request.method === "POST") {
      return await createReservation(request, env, context);
    }
    if (releaseMatch && request.method === "POST") {
      return await releaseReservation(request, env, releaseMatch[1] ?? "");
    }
    return new Response(null, {
      status: 405,
      headers: { "Cache-Control": "no-store", Allow: isWorkspace ? "GET, HEAD" : "POST" },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Reservation workflow failed";
    return redirect(request, `${WORKSPACE}?error=${encodeURIComponent(message.slice(0, 180))}`);
  }
}
