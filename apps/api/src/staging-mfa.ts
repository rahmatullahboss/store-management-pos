...
function safeReturnTo(value: FormDataEntryValue | null): string {
  if (typeof value !== "string") return "/admin/inventory/reservations";
  const normalized = value.trim();
  return normalized === "/admin/inventory/reservations"
    ? normalized
...
class PostgresStagingMfaStore implements StagingMfaStore {
  ...
  async verifyPassword(
    ...
    const rows = await this.database.httpQuery<{ valid: boolean } & Record<string, unknown>>(
  ...
  async activate(
    ...
    const rows = await this.database.httpQuery<{ activated: boolean } & Record<string, unknown>>(
  ...
  async consumeStepUp(
    ...
    const rows = await this.database.httpQuery<{ consumed: boolean } & Record<string, unknown>>(
...
export async function encryptTotpSecret(
  ...
  const ciphertext = new Uint8Array(
    await crypto.subtle.encrypt(
      { name: "AES-GCM", iv, additionalData: encoder.encode(`ozzyl-totp:${factorId}`) },
      key,
      secret,
    ),
  );
...
async function enroll(request: Request, env: StagingMfaEnvironment): Promise<Response> {
  ...
  if (!(await store(env).storePending({
...
async function confirm(request: Request, env: StagingMfaEnvironment): Promise<Response> {
  ...
  if (!factor) throw new PlatformError("NOT_FOUND", "Pending MFA enrollment was not found", 404);
  const secret = await decryptTotpSecret(factor, password);
  const counter = await verifyTotpCode(secret, code);
...
async function issueStepUp(request: Request, env: StagingMfaEnvironment): Promise<Response> {
  ...
  const grantExpires = new Date(Date.now() + STEP_UP_SECONDS * 1_000);
  const issued = await store(env).issueStepUp({
    ...
    counter,
    expiresAt: grantExpires.toISOString(),
    requestId: requestId(request),
  });
  if (!issued) {
    throw new PlatformError(
  ...
  return redirect(request, `${target.pathname}${target.search}`, stepUpCookie(grantToken, grantExpires));
}
...
export async function handleStagingMfaRequest(
  ...
  try {
    if (url.pathname === "/auth/mfa" && (request.method === "GET" || request.method === "HEAD")) {
    ...
    if (url.pathname === "/auth/mfa/status" && request.method === "GET") {
      ...
      const token = sessionToken(request);
      if (!context || !token) {
        return Response.json({ enrolled: false }, { status: 401, headers: { "Cache-Control": "no-store" } });
      }
      const factor = await store(env).loadFactor(await sha256(token), "active");
      return Response.json(
        {
          enrolled: factor !== null,
          factor: factor ? { label: factor.label, confirmedAt: factor.confirmedAt } : null,
