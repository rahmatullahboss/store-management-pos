import type { PriceTaxSnapshot } from "../../../../../modules/pricing/src/price-tax.js";
...
export interface PricingTaxAdminRenderOptions extends ModAAdminRenderOptions {
  readonly snapshot?: PriceTaxSnapshot;
}

const localeTags: Record<ModAAdminLocale, string> = { en: "en-GB", bn: "bn-BD", ar: "ar-SA", ja: "ja-JP" };

function renderExactMoney(amountMinor: bigint, currency: string, scale: number, locale: ModAAdminLocale): string {
  const absolute = amountMinor < 0n ? -amountMinor : amountMinor;
  const safeLimit = BigInt(Number.MAX_SAFE_INTEGER);
  if (absolute <= safeLimit) {
    const numeric = Number(amountMinor) / 10 ** scale;
  ...
  return `${amountMinor < 0n ? "-" : ""}${units.toString()}${scale === 0 ? "" : `.${fraction}`} ${currency}`;
}
...
export function renderPricingTaxAdmin(options: PricingTaxAdminRenderOptions = {}): string {
  ...
  const moneyText = (amountMinor: bigint, currency: string, scale: number) => renderExactMoney(amountMinor, currency, scale, locale);
  const unitPrice = snapshot === undefined ? "£8.00" : moneyText(snapshot.unitPriceMinor, snapshot.currency, snapshot.moneyScale);
  const subtotal = snapshot === undefined ? "£96.00" : moneyText(snapshot.subtotalMinor, snapshot.currency, snapshot.moneyScale);
  const promotedAmount = snapshot === undefined ? "£85.40" : moneyText(snapshot.promotedAmountMinor, snapshot.currency, snapshot.moneyScale);
  const net = snapshot === undefined ? "£80.00" : moneyText(snapshot.netMinor, snapshot.currency, snapshot.moneyScale);
  const tax = snapshot === undefined ? "£16.00" : moneyText(snapshot.taxMinor, snapshot.currency, snapshot.moneyScale);
  const gross = snapshot === undefined ? "£96.00" : moneyText(snapshot.grossMinor, snapshot.currency, snapshot.moneyScale);
  const hash = snapshot === undefined ? "7af2…d910" : `${snapshot.calculationHash.slice(0, 4)}…${snapshot.calculationHash.slice(-4)}`;
  const snapshotNotice = snapshot === undefined
    ? "Values demonstrate MOD-A controls only and are not production prices or customer data."
    : `Immutable snapshot ${snapshot.snapshotId} · calculation ${hash}. Values in the calculation panels come from this snapshot.`;
  const promotionRows = snapshot === undefined
    ? `<div class="moda-kv"><span>Subtotal</span><strong>${escapeHtml(subtotal)}</strong></div><div class="moda-kv"><span>SAVE10 · 10%</span><strong>−£9.60</strong></div><div class="moda-kv"><span>Category fixed discount</span><strong>−£1.00</strong></div><div class="moda-kv"><span>Rejected promotion</span><strong>Same stacking group</strong></div><div class="moda-kv moda-total"><span>Price after promotions</span><strong>${escapeHtml(promotedAmount)}</strong></div>`
    : `<div class="moda-kv"><span>Subtotal</span><strong>${escapeHtml(subtotal)}</strong></div>${snapshot.promotions.map((promotion) => `<div class="moda-kv"><span>${escapeHtml(promotion.promotionCode)} · v${promotion.version.toString()}</span><strong>−${escapeHtml(moneyText(promotion.discountMinor, snapshot.currency, snapshot.moneyScale))}</strong></div>`).join("") || '<div class="moda-empty-inline" role="status"><span aria-hidden="true">0</span><span>No promotion was applied to this immutable snapshot.</span></div>'}<div class="moda-kv moda-total"><span>Price after promotions</span><strong>${escapeHtml(promotedAmount)}</strong></div>`;
  const promotionState = snapshot === undefined ? statusChip("2 applied · 1 rejected", "info") : statusChip(`${snapshot.promotions.length} applied`, snapshot.promotions.length > 0 ? "info" : "neutral");
  const discountControl = snapshot === undefined
    ? `<div class="moda-kv"><span>Current price</span><strong>£85.40</strong></div><div class="moda-kv"><span>Requested discount</span><strong>£10.00</strong></div><div class="moda-kv"><span>Resulting price</span><strong>£75.40</strong></div><div class="moda-kv"><span>Automatic limit</span><strong>£8.54</strong></div><div class="moda-field"><label for="discount-reason">Reason</label><textarea id="discount-reason">Customer recovery after delayed fulfilment</textarea></div><div class="moda-actions"><button class="moda-button moda-button--danger" type="button">Reject</button><button class="moda-button moda-button--primary" type="button">Approve with audit</button></div>`
    : `<div class="moda-kv"><span>Snapshot discount</span><strong>${escapeHtml(moneyText(snapshot.discountMinor, snapshot.currency, snapshot.moneyScale))}</strong></div><div class="moda-kv"><span>Promoted amount</span><strong>${escapeHtml(promotedAmount)}</strong></div><div class="moda-empty-inline" role="status"><span aria-hidden="true">✓</span><span>This calculation is immutable. A later manual discount requires a new approval and a new snapshot.</span></div><button class="moda-button moda-button--secondary" type="button" disabled>No pending approval</button>`;
  const discountState = snapshot === undefined ? statusChip("Approval pending", "warning") : statusChip("Snapshot locked", "success");
  ...
  <section class="moda-shell" dir="${direction(locale)}" lang="${locale}" data-module="pricing-tax" data-state="${state}" aria-label="Pricing and tax control workspace">
    <div class="fixture-notice" role="note"><strong>${snapshot === undefined ? "Synthetic interface fixture" : "Immutable price-tax snapshot"}</strong><span>${escapeHtml(snapshotNotice)}</span></div>
    ...
    <section class="moda-workspace"><article class="moda-ledger"><header class="moda-ledger__head"><div><h2>${escapeHtml(text.ledger)}</h2><p>Precedence: customer group → channel → store → legal entity → global.</p></div>${statusChip("No effective overlap", "success")}</header>
      ...
      <aside class="moda-inspector" aria-label="${escapeHtml(text.inspector)}"><header class="moda-inspector__head"><div><h2>${escapeHtml(selected.name)}</h2><p>${escapeHtml(selected.code)} · selected version ${selected.version}</p></div>${statusChip(selected.state, selected.state === "Active" ? "success" : "warning")}</header>
        ...
        <div class="moda-inspector__body"><dl class="moda-detail-grid"><div><dt>Variant</dt><dd>${escapeHtml(snapshot?.variantId ?? "SHIRT-BLUE-M")}</dd></div><div><dt>Quantity</dt><dd>${snapshot === undefined ? "12 EA" : `${snapshot.quantityMinor.toString()} ${escapeHtml(snapshot.unitCode)}`}</dd></div><div><dt>Channel</dt><dd>POS</dd></div><div><dt>Store</dt><dd>Dhaka Central</dd></div><div><dt>Customer group</dt><dd>Retail</dd></div><div><dt>Business instant</dt><dd>${escapeHtml(snapshot?.calculatedAt ?? "28 Jul 2026 · 10:30")}</dd></div></dl>
          <h3 class="moda-divider-title">Winning rule</h3><div class="moda-kv"><span>Scope specificity</span><strong>Store + channel</strong></div><div class="moda-kv"><span>Quantity tier</span><strong>10+ EA</strong></div><div class="moda-kv"><span>Rule version</span><strong>v${snapshot?.priceRuleVersion.toString() ?? "3"} · immutable</strong></div>
          <h3 class="moda-divider-title">Exact result</h3><div class="moda-kv"><span>Unit price</span><strong>${escapeHtml(unitPrice)}</strong></div><div class="moda-kv"><span>Line subtotal</span><strong>${escapeHtml(subtotal)}</strong></div><div class="moda-kv"><span>Minimum margin price</span><strong>£7.50</strong></div><div class="moda-kv"><span>Calculation hash</span><strong>${escapeHtml(hash)}</strong></div>
    ...
    <section class="moda-lower"><article class="moda-simulator"><header class="moda-section-head"><div><h2>Promotion simulator</h2><p>Evaluate targeting, coupons, stacking groups and exact allocation before activation.</p></div>${promotionState}</header><div class="moda-simulator__body">${snapshot === undefined ? '<div class="moda-field"><label for="coupon-code">Coupon</label><div class="moda-inline-form"><input id="coupon-code" value="SAVE10"><button class="moda-button moda-button--secondary" type="button">Recalculate</button></div></div>' : ""}${promotionRows}<p class="moda-note">Every applied promotion retains its version and line-level allocation.</p></div></article>
      <article class="moda-strip"><header class="moda-section-head"><div><h2>Manual discount control</h2><p>Approval is required above the operator threshold or below the margin floor.</p></div>${discountState}</header><div class="moda-strip__body">${discountControl}</div></article></section>
    <section class="moda-lower"><article class="moda-strip"><header class="moda-section-head"><div><h2>Tax calculation snapshot</h2><p>Jurisdiction, treatment, rate versions and rounding remain explainable.</p></div>${statusChip("Reconciled", "success")}</header><div class="moda-strip__body"><div class="moda-kv"><span>Tax code</span><strong>${escapeHtml(snapshot?.taxCodeId ?? "VAT-STANDARD · v1")}</strong></div><div class="moda-kv"><span>Jurisdiction</span><strong>${escapeHtml(snapshot?.jurisdictionId ?? "GB · Standard")}</strong></div><div class="moda-kv"><span>Price mode</span><strong>${escapeHtml(snapshot?.taxPriceMode ?? "inclusive")}</strong></div><div class="moda-kv"><span>Net</span><strong>${escapeHtml(net)}</strong></div><div class="moda-kv"><span>Tax</span><strong>${escapeHtml(tax)}</strong></div><div class="moda-kv moda-total"><span>Gross</span><strong>${escapeHtml(gross)}</strong></div><p class="moda-note">Net + tax equals gross exactly in minor units. The immutable snapshot can be reused by returns.</p><button class="moda-button moda-button--secondary" type="button">Trace tax components</button></div></article>
