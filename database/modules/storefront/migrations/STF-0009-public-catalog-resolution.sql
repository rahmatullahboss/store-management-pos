BEGIN;

CREATE OR REPLACE FUNCTION storefront.format_public_quantity(
  p_amount numeric,
  p_scale integer
) RETURNS text
LANGUAGE plpgsql
IMMUTABLE
STRICT
SET search_path = pg_catalog AS $$
DECLARE
  v_digits text;
  v_padded text;
  v_whole text;
  v_fraction text;
BEGIN
  IF p_amount < 0 OR trunc(p_amount) <> p_amount OR p_scale < 0 OR p_scale > 18 THEN
    RAISE EXCEPTION 'invalid public quantity' USING ERRCODE = '22023';
  END IF;
  v_digits := trunc(p_amount)::text;
  IF p_scale = 0 THEN RETURN v_digits; END IF;
  v_padded := lpad(v_digits, p_scale + 1, '0');
  v_whole := left(v_padded, char_length(v_padded) - p_scale);
  v_fraction := rtrim(right(v_padded, p_scale), '0');
  RETURN CASE WHEN v_fraction = '' THEN v_whole ELSE v_whole || '.' || v_fraction END;
END $$;

CREATE OR REPLACE FUNCTION storefront.compose_public_product_documents(
  p_tenant_id uuid,
  p_storefront_id uuid,
  p_sales_channel_id uuid,
  p_locale text,
  p_currency text
) RETURNS TABLE(
  product_id uuid,
  public_slug text,
  product_document jsonb
)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = pg_catalog, storefront, platform, catalog, pricing, inventory
SET row_security = off AS $$
  WITH channel AS (
    SELECT
      sc.tenant_id,
      sc.storefront_id,
      sc.id AS sales_channel_id,
      sc.inventory_scope,
      sc.backorder_policy,
      sf.legal_entity_id,
      sf.primary_store_id,
      pl.id AS price_list_id,
      pl.currency::text AS currency,
      pl.money_scale,
      pl.active_version
    FROM storefront.sales_channels sc
    JOIN storefront.storefronts sf
      ON sf.tenant_id = sc.tenant_id
     AND sf.id = sc.storefront_id
     AND sf.status = 'active'
    JOIN pricing.price_lists pl
      ON pl.tenant_id = sc.tenant_id
     AND pl.id = sc.price_list_id
     AND pl.status = 'active'
     AND pl.active_version IS NOT NULL
    WHERE sc.tenant_id = p_tenant_id
      AND sc.storefront_id = p_storefront_id
      AND sc.id = p_sales_channel_id
      AND sc.status = 'active'
      AND pl.currency::text = upper(p_currency)
      AND pl.money_scale BETWEEN 0 AND 6
  ),
  price_version AS (
    SELECT plv.tenant_id, plv.id
    FROM pricing.price_list_versions plv
    JOIN channel c
      ON c.tenant_id = plv.tenant_id
     AND c.price_list_id = plv.price_list_id
     AND c.active_version = plv.version
    WHERE plv.status = 'active'
      AND plv.effective_from <= statement_timestamp()
      AND (plv.effective_until IS NULL OR plv.effective_until > statement_timestamp())
      AND (plv.legal_entity_id IS NULL OR plv.legal_entity_id = c.legal_entity_id)
      AND (plv.store_id IS NULL OR plv.store_id = c.primary_store_id)
      AND (plv.channel IS NULL OR plv.channel = 'web')
    ORDER BY plv.priority DESC, plv.version DESC, plv.id
    LIMIT 1
  ),
  warehouse_scope AS (
    SELECT DISTINCT w.id
    FROM channel c
    CROSS JOIN LATERAL jsonb_array_elements_text(
      CASE
        WHEN jsonb_typeof(c.inventory_scope -> 'warehouseIds') = 'array'
          THEN c.inventory_scope -> 'warehouseIds'
        ELSE '[]'::jsonb
      END
    ) raw(warehouse_id)
    JOIN platform.warehouses w
      ON raw.warehouse_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
     AND w.id = raw.warehouse_id::uuid
     AND w.tenant_id = p_tenant_id
     AND w.status = 'active'
  ),
  published_variants AS (
    SELECT
      pp.product_id,
      pp.public_slug,
      pp.metadata AS publication_metadata,
      p.code,
      p.kind,
      COALESCE(
        NULLIF(trim(localized.display_name), ''),
        NULLIF(trim(fallback_localized.display_name), ''),
        NULLIF(trim(p.code), ''),
        'Product'
      ) AS display_name,
      COALESCE(localized.description, fallback_localized.description) AS description,
      v.id AS variant_id,
      v.sku,
      COALESCE(NULLIF(trim(v.title), ''), p.code) AS variant_title,
      v.unit_code,
      u.decimal_scale AS quantity_scale,
      c.currency,
      c.money_scale,
      c.backorder_policy,
      (SELECT count(*) FROM warehouse_scope) AS warehouse_count,
      rule.unit_price_minor,
      rule.compare_at_price_minor,
      COALESCE(stock.on_hand, 0::numeric) AS on_hand,
      COALESCE(reservations.reserved, 0::numeric) AS reserved,
      COALESCE(stock.as_of, TIMESTAMPTZ '1970-01-01 00:00:00+00') AS inventory_as_of,
      COALESCE(stock.version, 0::bigint) AS inventory_version
    FROM storefront.product_publications pp
    JOIN catalog.products p
      ON p.tenant_id = pp.tenant_id
     AND p.id = pp.product_id
     AND p.status = 'active'
    LEFT JOIN catalog.product_localizations localized
      ON localized.tenant_id = p.tenant_id
     AND localized.product_id = p.id
     AND lower(localized.locale) = lower(p_locale)
    LEFT JOIN catalog.product_localizations fallback_localized
      ON fallback_localized.tenant_id = p.tenant_id
     AND fallback_localized.product_id = p.id
     AND lower(fallback_localized.locale) = lower(p.default_locale)
    JOIN catalog.variants v
      ON v.tenant_id = p.tenant_id
     AND v.product_id = p.id
     AND v.status = 'active'
     AND v.unit_code ~ '^[A-Za-z0-9][A-Za-z0-9._/-]{0,31}$'
    JOIN catalog.units u
      ON u.tenant_id = v.tenant_id
     AND u.code = v.unit_code
     AND u.status = 'active'
    LEFT JOIN storefront.variant_publications vp
      ON vp.tenant_id = pp.tenant_id
     AND vp.storefront_id = pp.storefront_id
     AND vp.sales_channel_id = pp.sales_channel_id
     AND vp.product_id = pp.product_id
     AND vp.variant_id = v.id
    JOIN channel c
      ON c.tenant_id = pp.tenant_id
     AND c.storefront_id = pp.storefront_id
     AND c.sales_channel_id = pp.sales_channel_id
    JOIN price_version version ON version.tenant_id = pp.tenant_id
    JOIN LATERAL (
      SELECT pr.unit_price_minor, pr.compare_at_price_minor
      FROM pricing.price_rules pr
      WHERE pr.tenant_id = pp.tenant_id
        AND pr.price_list_version_id = version.id
        AND pr.variant_id = v.id
        AND pr.unit_code = v.unit_code
        AND pr.quantity_scale = u.decimal_scale
        AND pr.minimum_quantity_minor <= power(10::numeric, u.decimal_scale)
        AND (pr.effective_from IS NULL OR pr.effective_from <= statement_timestamp())
        AND (pr.effective_until IS NULL OR pr.effective_until > statement_timestamp())
      ORDER BY pr.minimum_quantity_minor DESC, pr.priority DESC, pr.rule_version DESC, pr.id
      LIMIT 1
    ) rule ON true
    LEFT JOIN LATERAL (
      SELECT
        COALESCE(sum(sb.quantity_amount), 0::numeric) AS on_hand,
        max(sb.updated_at) AS as_of,
        max(sb.version) AS version
      FROM inventory.stock_balances sb
      JOIN warehouse_scope ws ON ws.id = sb.warehouse_id
      WHERE sb.tenant_id = pp.tenant_id
        AND sb.variant_id = v.id
        AND sb.stock_status = 'sellable'
        AND sb.unit_code = v.unit_code
        AND sb.quantity_scale = u.decimal_scale
    ) stock ON true
    LEFT JOIN LATERAL (
      SELECT COALESCE(
        sum(line.reserved_quantity - line.consumed_quantity - line.released_quantity),
        0::numeric
      ) AS reserved
      FROM inventory.stock_reservation_lines line
      JOIN inventory.stock_reservations reservation
        ON reservation.tenant_id = line.tenant_id
       AND reservation.id = line.reservation_id
       AND reservation.state IN ('fully_reserved','partially_reserved','partially_consumed')
      JOIN warehouse_scope ws ON ws.id = line.warehouse_id
      WHERE line.tenant_id = pp.tenant_id
        AND line.variant_id = v.id
        AND line.unit_code = v.unit_code
        AND line.quantity_scale = u.decimal_scale
    ) reservations ON true
    WHERE pp.tenant_id = p_tenant_id
      AND pp.storefront_id = p_storefront_id
      AND pp.sales_channel_id = p_sales_channel_id
      AND pp.publication_state = 'published'
      AND COALESCE(vp.publication_state, 'published') = 'published'
  ),
  variant_state AS (
    SELECT
      source.*,
      GREATEST(source.on_hand - source.reserved, 0::numeric) AS available_quantity,
      CASE
        WHEN source.kind IN ('service','non_stock') THEN 'available'
        WHEN source.warehouse_count = 0 THEN 'unknown'
        WHEN source.on_hand - source.reserved > 0 THEN 'available'
        WHEN source.backorder_policy IN ('allow','preorder_only') THEN 'preorder'
        ELSE 'unavailable'
      END AS availability
    FROM published_variants source
  ),
  variant_documents AS (
    SELECT
      state.*,
      jsonb_build_object(
        'variantId', state.variant_id::text,
        'sku', left(COALESCE(NULLIF(trim(state.sku), ''), state.variant_id::text), 160),
        'title', left(state.variant_title, 240),
        'unitCode', state.unit_code,
        'availability', state.availability,
        'price', jsonb_build_object(
          'currency', state.currency,
          'minor', state.unit_price_minor::text,
          'scale', state.money_scale
        ),
        'compareAtPrice', CASE
          WHEN state.compare_at_price_minor IS NULL THEN NULL
          ELSE jsonb_build_object(
            'currency', state.currency,
            'minor', state.compare_at_price_minor::text,
            'scale', state.money_scale
          )
        END,
        'quantity', CASE
          WHEN state.kind IN ('service','non_stock') OR state.warehouse_count = 0 THEN NULL
          ELSE jsonb_build_object(
            'amount', storefront.format_public_quantity(
              state.available_quantity,
              state.quantity_scale
            ),
            'unit', state.unit_code,
            'scale', state.quantity_scale,
            'asOf', to_char(
              state.inventory_as_of AT TIME ZONE 'UTC',
              'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
            ),
            'version', state.inventory_version::text
          )
        END
      ) AS variant_document
    FROM variant_state state
  )
  SELECT
    variant.product_id,
    min(variant.public_slug),
    jsonb_build_object(
      'summary', jsonb_build_object(
        'contractVersion', 'storefront-product-card.v1',
        'productId', variant.product_id::text,
        'variantId', (
          array_agg(
            variant.variant_id
            ORDER BY variant.unit_price_minor, variant.variant_id
          )
        )[1]::text,
        'slug', min(variant.public_slug),
        'name', left(min(variant.display_name), 240),
        'publicationState', 'published',
        'availability', CASE
          WHEN bool_or(variant.availability = 'available') THEN 'available'
          WHEN bool_or(variant.availability = 'preorder') THEN 'preorder'
          WHEN bool_and(variant.availability = 'unavailable') THEN 'unavailable'
          ELSE 'unknown'
        END,
        'pricePrefix', CASE
          WHEN count(DISTINCT variant.unit_price_minor) > 1 THEN 'from'
          ELSE 'none'
        END,
        'price', jsonb_build_object(
          'currency', min(variant.currency),
          'minor', min(variant.unit_price_minor)::text,
          'scale', min(variant.money_scale)
        ),
        'compareAtPrice', CASE
          WHEN (
            array_agg(
              variant.compare_at_price_minor
              ORDER BY variant.unit_price_minor, variant.variant_id
            )
          )[1] IS NULL THEN NULL
          ELSE jsonb_build_object(
            'currency', min(variant.currency),
            'minor', (
              array_agg(
                variant.compare_at_price_minor
                ORDER BY variant.unit_price_minor, variant.variant_id
              )
            )[1]::text,
            'scale', min(variant.money_scale)
          )
        END,
        'media', NULL,
        'badge', CASE
          WHEN char_length(trim(min(variant.publication_metadata ->> 'badge'))) BETWEEN 1 AND 80
           AND trim(min(variant.publication_metadata ->> 'badge')) !~ '[[:cntrl:]]'
            THEN trim(min(variant.publication_metadata ->> 'badge'))
          ELSE NULL
        END
      ),
      'code', left(min(variant.code), 160),
      'description', CASE
        WHEN min(variant.description) IS NULL THEN NULL
        ELSE left(min(variant.description), 5000)
      END,
      'kind', min(variant.kind),
      'pricingNotice', 'tax_calculated_at_checkout',
      'variants', jsonb_agg(
        variant.variant_document
        ORDER BY variant.unit_price_minor, variant.variant_id
      )
    )
  FROM variant_documents variant
  GROUP BY variant.product_id
  ORDER BY variant.product_id;
$$;

REVOKE ALL ON FUNCTION storefront.format_public_quantity(numeric,integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION storefront.compose_public_product_documents(uuid,uuid,uuid,text,text) FROM PUBLIC;

INSERT INTO platform.schema_migrations(migration_id, module, checksum)
VALUES ('STF-0009','MOD-H-STOREFRONT','manifest:STF-0009-public-catalog-resolution.sql')
ON CONFLICT (migration_id) DO NOTHING;

COMMIT;
