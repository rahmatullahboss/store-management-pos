...
BEGIN
  ...
  IF btrim(p_reason) = ''
     ...
     OR cardinality(p_scopes) <> cardinality(ARRAY(SELECT DISTINCT value FROM unnest(p_scopes) AS value))
     OR EXISTS (SELECT 1 FROM unnest(p_scopes) AS value WHERE value !~ '^[a-z][a-z0-9_.-]{2,127}$') THEN
...
BEGIN
  ...
  IF v_grant.support_actor_id IS DISTINCT FROM p_support_actor_id
     OR p_observed_at < v_grant.issued_at
     OR p_observed_at >= v_grant.expires_at
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    SELECT g.version INTO v_next_version
    FROM platform.support_impersonation_grants AS g
    WHERE g.tenant_id = p_tenant_id AND g.id = p_grant_id;
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    SELECT r.version INTO v_next_version
    FROM platform.feature_rollouts AS r
    WHERE r.tenant_id = p_tenant_id AND r.id = p_rollout_id;
...
BEGIN
  ...
  INSERT INTO platform.outbox_events(
    ...
    jsonb_build_object('incidentCode', p_incident_code, 'severity', p_severity, 'status', 'open'),
    jsonb_build_object('requestId', p_request_id, 'traceId', p_trace_id, 'source', 'mod-g'),
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    SELECT i.version INTO v_next_version
    FROM platform.support_incidents AS i
    WHERE i.tenant_id = p_tenant_id AND i.id = p_incident_id;
  ...
  SET status = p_new_status,
      resolved_at = CASE WHEN p_new_status = 'resolved' THEN p_observed_at
                         WHEN p_new_status = 'investigating' THEN NULL
