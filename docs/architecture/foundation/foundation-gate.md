...
| Direct Neon HTTP and request-scoped WebSocket adapters | Code/build/unit pass; live connectivity pending | Pinned driver installs in CI; adapters and cleanup/failure tests pass; credentialed HTTP/WS benchmark awaits Neon CI secrets |
...
| Backup/restore | Partial | Fresh empty-database rebuild and cleanup passed; PITR/history restore and reconciliation remain pending |
