...
const embeddedMainClasses = ["mod-c-workspace", "mod-c-sales", "mod-c-fulfilment", "pos-reconciliation"];

function normalizeEmbeddedMain(html, rootClass) {
...
function renderScenario(scenario) {
  ...
  return embeddedMainClasses.reduce(
    (html, rootClass) => normalizeEmbeddedMain(html, rootClass),
    rendered,
  );
}
...
try {
  for (const scenario of scenarios) {
    ...
    const layout = await page.evaluate(() => {
      ...
      const rootOverflow = root.scrollWidth > innerWidth + 2;
      return {
        clipped,
        rootOverflow,
    ...
    const markerPresent = html.includes(scenario.marker);
