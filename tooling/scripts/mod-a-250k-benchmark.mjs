...
if (!connectionString) throw new Error("DATABASE_URL is required for the MOD-A PostgreSQL benchmark");
...
try {
  ...
  await client.query("CREATE INDEX benchmark_barcode_idx ON mod_a_benchmark.variant_search_documents USING gin(barcodes)");
  ...
  const exactBarcode = await measureQuery(client,
    "SELECT variant_id, normalized_sku FROM mod_a_benchmark.variant_search_documents WHERE tenant_id=$1 AND barcodes @> ARRAY[$2]::text[] LIMIT 20",
  ...
  const combinedSearch = await measureQuery(client,
    `SELECT variant_id, normalized_sku
     FROM mod_a_benchmark.variant_search_documents
     WHERE tenant_id=$1 AND status='active' AND (
       normalized_sku=upper(btrim($2)) OR
       barcodes @> ARRAY[upper(btrim($2))]::text[] OR
       search_vector @@ plainto_tsquery('simple'::regconfig,$2) OR
       searchable_text OPERATOR(public.%) $2
     )
     ORDER BY CASE
       WHEN normalized_sku=upper(btrim($2)) THEN 0
       WHEN barcodes @> ARRAY[upper(btrim($2))]::text[] THEN 1
       ELSE 2
     END, updated_at DESC
     LIMIT 20`,
    [tenantId, "SKU-249999"],
  ...
  report = {
    ...
    budgets,
    import: { rows: variantCount, elapsedMs: Number(importMs.toFixed(2)), rowsPerSecond: Number((variantCount / (importMs / 1000)).toFixed(2)) },
...
const markdown = `# MOD-A 250,000-Variant PostgreSQL Performance Report\n\n**Generated:** ${report.generatedAt}\n**Status:** ${report.status}\n**Neon project:** \`${report.neonProjectId}\`\n**Neon branch:** \`${report.neonBranchId}\`\n\nThe benchmark used a disposable unlogged PostgreSQL table with the same tenant, SKU, barcode-array, generated tsvector and trigram search shapes used by the catalog projection. The fixture was dropped after evidence capture. No production data or default branch was changed.\n\n## Import\n\n- Representative variants: ${report.import.rows.toLocaleString('en-GB')}\n- Elapsed: ${report.import.elapsedMs} ms\n- Throughput: ${report.import.rowsPerSecond} rows/second\n- Budget: ${report.budgets.importMaxMs} ms\n\n## Search latency\n\n| Query | Rows returned | p50 ms | p95 ms | p99 ms | max ms |\n|---|---:|---:|---:|---:|---:|\n${queryRows}\n\n## Gate checks\n\n| Check | Result |\n|---|---|\n${checkRows}\n\nExecution plans and machine-readable latency distributions are retained in [performance-report.json](performance-report.json).\n`;
