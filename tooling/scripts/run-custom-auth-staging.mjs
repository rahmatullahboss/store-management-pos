  ...
  try {
    const rows = await query.query(
      ...
      [
        ...
        ipHash,
        userAgentHash,
        `custom-auth-http-preflight-${suffix}`,
      ],
    );
    if (rows.length !== 1 || typeof rows[0]?.user_id !== "string") {
      throw new Error("Custom auth Neon HTTP preflight returned an invalid row");
    }
    userId = rows[0].user_id;
    console.log("custom auth Neon HTTP preflight passed");
  } catch (error) {
    const record =
