...
const {
  GITHUB_RUN_ID,
  GITHUB_SHA,
  MOD_F_NEON_BRANCH_ID = "br-polished-flower-ax2ph8wp",
  NEON_API_KEY,
  NEON_PROJECT_ID = "twilight-boat-26805962",
} = process.env;

if (!NEON_API_KEY) throw new Error("NEON_API_KEY is required for the MOD-F Neon rehearsal");

const root = fileURLToPath(new URL("../..", import.meta.url));
const artifactsDirectory = path.join(root, "artifacts", "mod-f");
...
const advisoryLockName = `store-management-pos:mod-f-neon-rehearsal:${NEON_PROJECT_ID}:${MOD_F_NEON_BRANCH_ID}`;
...
const report = {
  ...
  branchId: MOD_F_NEON_BRANCH_ID,
  advisoryLockName,
  advisoryLockAcquired: false,
  migrationIds: [],
  localizationTables: 0,
  forcedRlsTables: 0,
  runtimeWriteGrants: 0,
  publicExecuteGrants: 0,
  integrationWorkflowPassed: false,
  failure: null,
};
...
try {
  const branchResponse = await api(`/branches/${encodeURIComponent(MOD_F_NEON_BRANCH_ID)}`);
  const branch = branchResponse.branch || branchResponse;
  if (branch.name !== "dev/module-localization-compliance") {
    throw new Error(`MOD-F branch identity mismatch: ${branch.name || "unknown"}`);
  }
  if (branch.default || branch.primary || !branch.parent_id) {
    throw new Error("MOD-F rehearsal branch must be an isolated non-default child branch");
  }

  const uriResponse = await api(`/connection_uri?branch_id=${encodeURIComponent(MOD_F_NEON_BRANCH_ID)}&database_name=neondb&role_name=neondb_owner`);
  const connectionString = uriResponse.uri;
  if (typeof connectionString !== "string") throw new Error("Neon API did not return a MOD-F connection URI");
  ...
  console.log(`acquired MOD-F Neon rehearsal advisory lock ${advisoryLockName}`);
  ...
  if (missing.length > 0) throw new Error(`MOD-F rehearsal is missing migrations: ${missing.join(", ")}`);

  const tables = await client.query(`
    SELECT count(*)::integer AS localization_tables
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'localization' AND c.relkind = 'r'
  `);
  const rls = await client.query(`
    SELECT count(*)::integer AS forced_rls_tables
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'localization'
      AND c.relkind = 'r'
  ...
  report.localizationTables = tables.rows[0]?.localization_tables ?? 0;
  ...
  if (report.localizationTables === 0) throw new Error("MOD-F rehearsal found no localization tables");
  if (report.forcedRlsTables !== report.localizationTables) throw new Error("Not every MOD-F table has forced RLS");
  if (report.runtimeWriteGrants !== 0) throw new Error("store_app_runtime has direct MOD-F table write grants");
  if (report.publicExecuteGrants !== 0) throw new Error("PUBLIC execute privilege remains on a MOD-F function");

  await run("npm", ["run", "test:integration"], {
    ...process.env,
    DATABASE_URL: connectionString,
    MOD_F_NEON_INTEGRATION: "1",
  });
  report.integrationWorkflowPassed = true;

  report.status = "passed";
  console.log(`MOD-F Neon rehearsal passed on ${MOD_F_NEON_BRANCH_ID}`);
} catch (error) {
  ...
  if (client) {
    if (advisoryLockAcquired) {
      try {
        ...
        console.log(`released MOD-F Neon rehearsal advisory lock ${advisoryLockName}`);
      } catch (unlockError) {
